...
if(isset($_POST['add_to_cart'])){
   ...
   if(mysqli_num_rows($check_cart_numbers) > 0){
      ...
      mysqli_query($conn, "INSERT INTO `cart`(user_id, name, price, quantity, image) VALUES('$user_id', '$product_name', '$product_price', '$product_quantity', '$product_image')") or die('Lỗi truy vấn');
...
<section class="products" style="padding-top: 0;">
   ...
   <?php
      if(isset($_POST['submit'])){
         ...
         $select_products = mysqli_query($conn, "SELECT * FROM `products` WHERE name LIKE '%{$search_item}%'") or die('Lỗi truy vấn');
   ...
   <?php
            ...
            echo '<p class="empty">Không có kết quả nào!</p>';
         }
      }else{
         echo '<p class="empty">Hãy tìm kiếm một sản phẩm!</p>';
      }
   ?>
   </div>
